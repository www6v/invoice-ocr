...
def main(filepath):
    ...
    if response.status_code == 200:
        ...
        res_data = []
        for data in response.json()['result']['invoiceList']:
...
with gr.Blocks(css="styles.css") as demo:
    with gr.Row():
        with gr.Column(scale=1):
            ...
            gr.Markdown("""
                1.**上传发票文档-pdf/图片**  
                2.**输出关键信息**：发票类型，发票代码，发票号码，发票日期，销售方名称/识别号，购买方名称/识别号，含税总金额，不含税总金额 
            """)
...
if __name__ == "__main__":
    demo.launch(server_port=9100, server_name="0.0.0.0")